...
export default function BookingForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (isShuttleExclusiveSlot(data.timeSlot) && data.shuttle !== "on") {
      setErrorMsg(
        "That time is reserved for shuttle bookings only. Please choose another time or check the shuttle pickup option.",
      );
      setStatus("error");
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <div>
          ...
          {TIME_SLOTS.map((slot) => {
            ...
            return (
              <option
                ...
                value={slot}
                disabled={
                  bookedSlots.includes(slot) || isCooldownSlot(slot) || shuttleOnly
                }
              >
                {formatTimeSlot(slot)}
                {isCooldownSlot(slot)
        ...
        <p className="font-body text-grit-muted text-xs mt-1">
          ...
          4:00–4:15pm) are marked and can&apos;t be selected. 2:00–3:30pm is
          reserved for shuttle bookings only.
        </p>
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            ...
            onChange={(e) => {
              ...
              if (!checked && isShuttleExclusiveSlot(timeSlot)) setTimeSlot("");
            }}
            disabled={isSubmitting}
            className="mt-1 accent-grit-orange"
          />
